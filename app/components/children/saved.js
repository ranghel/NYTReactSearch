...
var Saved = React.createClass({
    ...
    componentWillReceiveProps: function(nextProps){
        ...
        var myResults = nextProps.savedArticles.map(function(search, i){
            ...
            return <div className="list-group-item" key={i}><a href={search.url} target="_blank">{search.title}</a><br />{search.date}<br /><button type="button" className="btn btn-danger" style={{'float': 'right', 'marginTop': '-39px'}} onClick={boundClick}>Remove</button></div>
        });
